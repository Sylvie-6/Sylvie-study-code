...
# os.environ['HF_ENDPOINT'] = 'https://hf-mirror.com'  # 设置HuggingFace镜像，解决模型下载问题
os.environ['OLLAMA_MODELS'] = r'D:\ollama\Ollama_models'
...
from typing import List, Tuple, Any, Optional  # 类型提示

# === 全局配置变量 ===
load_dotenv()  # 加载.env环境变量文件

# API密钥配置
SERPAPI_KEY = os.getenv("SERPAPI_KEY")  # SerpAPI搜索服务密钥
SILICONFLOW_API_KEY = os.getenv("SILICONFLOW_API_KEY")  # 硅流云端模型API密钥
SILICONFLOW_API_URL = os.getenv("SILICONFLOW_API_URL",
                                "https://api.siliconflow.cn/v1/chat/completions")  # 硅流API地址

# 搜索引擎配置
SEARCH_ENGINE = "google"  # 默认搜索引擎
RERANK_METHOD = os.getenv("RERANK_METHOD", "cross_encoder")  # 重排序方法：cross_encoder或llm

# 性能优化配置
requests.adapters.DEFAULT_RETRIES = 3  # 增加HTTP请求重试次数
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # 禁用TensorFlow oneDNN优化
os.environ['NO_PROXY'] = 'localhost,127.0.0.1'  # 绕过本地代理设置

# === 核心组件初始化 ===
EMBED_MODEL = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2')  # 文本向量化模型
# 注: 可替换为中文优化模型 'shibing624/text2vec-base-chinese'

# FAISS向量数据库相关全局变量
faiss_index = None  # FAISS向量索引对象
faiss_contents_map = {}  # 文档内容映射 {chunk_id: content}
faiss_metadatas_map = {}  # 文档元数据映射 {chunk_id: metadata}
faiss_id_order_for_index = []  # FAISS索引中ID的顺序记录

# 交叉编码器延迟加载相关
cross_encoder = None  # 交叉编码器模型对象
cross_encoder_lock = threading.Lock()  # 线程锁，确保安全加载
...
def get_cross_encoder():
    ...
    返回:
        CrossEncoder: 交叉编码器模型实例，加载失败时返回None
    """
    global cross_encoder
    if cross_encoder is None:
        with cross_encoder_lock:
            if cross_encoder is None:  # 双重检查锁定模式
                try:
                    # 使用多语言交叉编码器，对中文支持更好
                    cross_encoder = CrossEncoder('sentence-transformers/distiluse-base-multilingual-cased-v2')
                    logging.info("交叉编码器模型加载成功")
                except Exception as e:
                    logging.error(f"交叉编码器加载失败: {str(e)}")
                    cross_encoder = None  # 设置为None，支持重试
    return cross_encoder


# 新增：BM25索引管理
#先用当前查询取回候选文档（语义+BM25混检并重排），
#再让模型判断是否需要基于当前已获信息生成“更好的下一轮查询”，循环迭代，
#直到足够或达到轮次上限。返回累计的上下文、对应文档ID与元数据。
def recursive_retrieval(initial_query, max_iterations=3, enable_web_search=False, model_choice="ollama"):
    ...
    for i in range(max_iterations):
        ...
        #把当前查询用 EMBED_MODEL 编码成向量（供 FAISS 检索）。
        #准备三个空列表，用来装本轮 FAISS 检索到的文档内容、元数据、原始ID。
        ...
        if docs_current_iter:
            try:
                ...
                # 打印交叉编码器重排序后的文档和得分
                print("\n=== 交叉编码器重排序后文档得分预览 ===")
                for idx, (doc_id, result_data) in enumerate(reranked_results):
                    content_preview = result_data['content'][:100].replace('\n', ' ')
                    print(
                        f"{idx + 1}. doc_id: {doc_id} | score: {result_data['score']:.4f} | preview: {content_preview}")
                print("=======================================\n")
        ...
        #是否继续下一轮检索”的决策与生成下一轮查询（递归检索的核心控制逻辑）
...
[输出]"""
            ...
            #会把捕获到的异常对象绑定到变量 e，便于后续打印日志、读取错误信息等。
            except Exception as e:
...
class BM25IndexManager:
    def __init__(self):
        ...
        self.tokenized_corpus = []# 存储分词后的文档集合
        self.raw_corpus = []# 存储原始文档内容
    ...
    def build_index(self, documents, doc_ids):
        ...
        # 创建BM25索引
        self.bm25_index = BM25Okapi(self.tokenized_corpus)#使用分词后的语料库创建BM25Okapi索引对象
        return True

    def search(self, query, top_k=5):#搜索方法
        """使用BM25检索相关文档"""
    ...
    try:
        ...
        if not knowledge_base_exists:
            if not enable_web_search:
                ...
                #yield 是用于定义生成器（generator）的关键字，它的作用类似于 return
                #但不会终止函数执行，而是暂停函数并返回一个值，下次调用时会从暂停的位置继续执行。
                return
            else:
...
请现在开始回答："""
        ...
        if model_choice == "siliconflow":
            ...
            for line in response.iter_lines():#iter_lines() 是 requests 库中 Response 对象的方法，用于逐行读取流式响应。
                if line:
...
def hybrid_merge(semantic_results, bm25_results, alpha=0.7):
    ...
    if (semantic_results and
        ...
        for i, (doc_id, doc, meta) in enumerate(
            ...
            score = 1.0 - (i / max(1, num_results))  # 给每个文档分配一个分数，排名越靠前分数越高
            merged_dict[doc_id] = {
                'score': alpha * score,
                'content': doc,
                'metadata': meta
    ...
    #遍历BM25结果，处理每个结果，每个 result 是一个字典，包含文档的id、分数、内容等。
