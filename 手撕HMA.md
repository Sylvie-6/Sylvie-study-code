...
| 名称         | 公式/操作                              | 形状                                       |
| ---------- | ---------------------------------- | ---------------------------------------- |
| `q`        | 输入                                 | `(b, n_q, dimension_k)` = `(10, 4, 128)` |
| `k`        | 输入                                 | `(b, n_k, dimension_k)` = `(10, 4, 128)` |
| `v`        | 输入                                 | `(b, n_v, dimension_v)` = `(10, 4, 64)`  |
| `fc_q(q)`  | 线性                                 | `(b, n_q, h*d_k)` = `(10, 4, 128)`       |
| `fc_k(k)`  | 线性                                 | `(b, n_k, h*d_k)` = `(10, 4, 128)`       |
| `fc_v(v)`  | 线性                                 | `(b, n_v, h*d_v)` = `(10, 4, 128)`       |
| `Q`        | `view(b,n_q,h,d_k).transpose(1,2)` | `(b, h, n_q, d_k)` = `(10, 8, 4, 16)`    |
| `K`        | 同上                                 | `(10, 8, 4, 16)`                         |
| `V`        | 同上（`d_v`）                          | `(10, 8, 4, 16)`                         |
| `scores`   | `Q @ K^T / sqrt(d_k)`              | `(b, h, n_q, n_k)` = `(10, 8, 4, 4)`     |
| `mask`     | `unsqueeze(1)`                     | `(b, 1, n_q, n_k)` = `(10, 1, 4, 4)`     |
| `attn`     | `softmax(scores+mask)`             | `(10, 8, 4, 4)`                          |
| `head_out` | `attn @ V`                         | `(10, 8, 4, 16)`                         |
| `concat`   | `transpose→view`                   | `(10, 4, 8*16)` = `(10, 4, 128)`         |
| `out`      | `fc_o(concat)`                     | `(b, n_q, d_o)` = `(10, 4, 8)`           |
